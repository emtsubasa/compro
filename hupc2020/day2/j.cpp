...
int main() {
  ...
  a.resize(n);
  for (auto &p : a) cin >> p;
  auto f = [](long long ln, long long rn) { return max(ln, rn); };
  SegmentTree<long long> seg, dp;
  seg = SegmentTree<long long>(a, f, (long long)-1e10);
  dp = SegmentTree<long long>(n + 1, f, (long long)-1e10);
  dp.update(0, 0);
  for (int i = l; i <= n; ++i) {
    long long now = dp.query(0, i - l + 1);
    now += seg.query(i - l, i);
    dp.update(i, now);
  }
