...
bool solve() {
  if (n <= 3) return 1;
  for (int i = 2; i < n; ++i)
    if (abs(calc_clockwiseSP(v[0], v[1], v[i])) == 1) swap(v[2], v[i]);
  if (abs(calc_clockwiseSP(v[0], v[1], v[2])) != 1) return 1;
  auto check = [](CP a, CP b, CP c) {
    CP d = c + b - a;
    for (int i = 0; i < n; ++i)
      if (abs(calc_clockwiseSP(a, b, v[i])) == 1 &&
          abs(calc_clockwiseSP(c, d, v[i])) == 1)
        return 0;
    return 1;
  };
  return check(v[0], v[1], v[2]) || check(v[1], v[2], v[0]) ||
         check(v[2], v[0], v[1]);
}
